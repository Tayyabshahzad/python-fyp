# Bilayer MoS2 Fig. 11 -- Hall conductivity sigma_yx vs B, T=1K, V=15meV.
# "As in Fig. 10 but for V = 15 meV" (paper's own framing, p.9: "We plot the
# Hall conductivity versus the field B in Fig. 11 for electric field energy
# V = 15 meV"). Reuses the EXACT same build_levels/fermi_energy/sigma_yx
# code as Codes/10.py (Eq. 22 + Appendix A), just with V=0.015 instead of 0.
#
# STATUS: same caveat as Codes/10.py -- envelope/magnitude of sigma_yx is
# expected to be trustworthy (same verified formula), but the quantized
# plateau/staircase structure the paper shows is NOT reproduced -- this
# implementation gives a smooth curve instead. See Codes/10.py's header for
# the FULL investigation record: two rounds of debugging (level-crowding
# diagnostic, mu-pairing restriction test, NMAX convergence test, ultra-fine
# B-resolution scan, and a full manual re-derivation of Eq. 6/7/22/23/24 +
# Appendix A against the primary PDF text, character by character) -- none
# of it found a bug or resolved the missing plateaus. NOT resolved. This
# file was previously entirely MISSING from Codes/ -- built this session to
# complete the 14-figure set; do not treat it as more validated than Fig.10.
import numpy as np
...
if __name__ == "__main__":
    ...
    for ax, B_range, B_inset, inset_pos in panels:
        ...
        ax.text(0.05, 0.08, "V = 15 meV", transform=ax.transAxes, fontsize=9)
    ...
    fig.suptitle(
        r"Hall conductivity $\sigma_{xy}$ vs $B$ at $T=1$ K, $V=15$ meV. "
        r"NOTE: envelope/magnitude expected reliable (same formula as Fig. 10), "
        r"but quantized plateau structure is NOT reproduced here.",
        fontsize=9
    )
    fig.tight_layout()
    plt.savefig("bilayer_MoS2_fig11_draft.png", dpi=200)
    print("Saved: bilayer_MoS2_fig11_draft.png")
